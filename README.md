...
- **3 modes d'affichage sur Vue d'ensemble, Activité du jour, Tâches, Planning, FDR et Rapport** : les mêmes données, présentées différemment selon ce qu'on cherche à voir — le sélecteur (en haut de chaque onglet) retient le choix de chaque personne (stockage local du navigateur, jamais partagé entre collègues). Changer de mode ne change jamais ce qu'on peut faire : les modes des onglets modifiables restent modifiables (affecter une tâche, changer un statut, saisir du temps), aucun n'est une vue morte en lecture seule.
