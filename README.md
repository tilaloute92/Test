...
- **Vue d'ensemble** : en haut de page, un bloc **"Points d'attention"** regroupe en un seul endroit ce qui, sinon, demanderait de visiter plusieurs onglets — incidents critiques/hauts ouverts, tâches en retard, personnes en surcharge, projets en flash report Rouge/Orange, initiatives FDR pas démarrées alors que leur trimestre est atteint, actions de COPIL en retard et séances de COPIL proches dont l'ordre du jour est encore vide. **Chaque ligne se déplie sur place** pour voir directement *lesquels* sans changer d'onglet : le détail des incidents (titre, statut, priorité, assigné[s]), des tâches en retard (avec leur échéance), des personnes en surcharge (heures planifiées vs disponibles), des projets Rouge/Orange (avec les raisons du statut) et des initiatives FDR concernées. Un bouton "Ouvrir →" reste disponible sur chaque ligne pour aller agir dans l'onglet d'origine. Rien à signaler affiche un message vert plutôt qu'un bloc vide ; à l'impression, tous les détails sont dépliés d'office. Ensuite : charge de la semaine par personne (sous-chargé / équilibré / chargé / surchargé) pour repérer un déséquilibre et rééquilibrer la répartition, tâche en cours de chacun, indicateurs (incidents ouverts, tâches en retard, présence du jour).
...
- **COPIL (comités de pilotage)** : les séances de gouvernance avec les parties prenantes (direction, métiers, prestataires) — l'instance qui arbitre, à distinguer de la FDR (le *quoi* sur l'année) et du planning (le *qui fait quoi* sur la semaine). Chaque séance porte sa date/heure/lieu, son statut (Planifié / Tenu / Annulé), ses participants (membres de l'équipe **et** intervenants extérieurs, qui ne sont pas comptés dans la charge de l'équipe), son **ordre du jour** (avec présentateur et durée, total affiché pour vérifier que la séance tient dans le temps imparti), les **initiatives FDR passées en revue** (lien de traçabilité, avancement affiché), les **décisions actées** et le **relevé d'actions** (responsable(s), échéance, statut) suivi d'une séance à l'autre. Une action de COPIL est volontairement distincte d'une tâche : c'est un engagement pris devant les parties prenantes, pas une unité de travail planifiable sur un créneau. Export du compte-rendu en Markdown (presse-papiers), impression/PDF, ou **PowerPoint** (le support qu'on projette en séance : titre, ordre du jour, FDR revue, décisions, relevé d'actions, notes et prochaine séance). Les actions en retard et les séances proches dont l'ordre du jour est encore vide remontent automatiquement dans les **Points d'attention** de la Vue d'ensemble.
...
- **3 modes d'affichage sur Vue d'ensemble, Activité du jour, Tâches, Planning, FDR, COPIL et Rapport** : les mêmes données, présentées différemment selon ce qu'on cherche à voir — le sélecteur (en haut de chaque onglet) retient le choix de chaque personne (stockage local du navigateur, jamais partagé entre collègues). Changer de mode ne change jamais ce qu'on peut faire : les modes des onglets modifiables restent modifiables (affecter une tâche, changer un statut, saisir du temps), aucun n'est une vue morte en lecture seule.
  - **Vue d'ensemble** : Complet (charge détaillée + une fiche par personne, vue par défaut) / Compact (un tableau dense, une ligne par personne : charge, tâche en cours, tâches ouvertes) / Charge 3 semaines (heures planifiées vs capacité réellement disponible par personne et par semaine — pour voir venir une surcharge avant qu'elle n'arrive).
  - **Activité du jour** : Par personne (une fiche par personne avec ses deux créneaux, vue par défaut) / Par créneau (toute l'équipe côte à côte, le matin d'un côté et l'après-midi de l'autre) / Tableau (une ligne par personne, une colonne par créneau + le temps saisi).
  ...
  - **COPIL** : Séances (liste des séances + détail complet de celle sélectionnée, vue par défaut — la prochaine séance à venir s'ouvre d'office) / Actions (toutes les actions de toutes les séances au même endroit, filtrables par statut et par responsable, les retards en tête) / Calendrier (les séances dans l'ordre chronologique, à venir puis passées).
...
**Export & impression sur chaque onglet** : un bouton "Imprimer / PDF" (menu Imprimer du navigateur, choisir "Enregistrer en PDF" pour exporter en fichier) est disponible en haut de chaque onglet — les éléments d'interface (filtres, boutons d'action, formulaires) sont masqués à l'impression pour ne garder que le contenu, et le thème sombre est automatiquement désactivé le temps de l'impression pour rester lisible sur papier. En complément, l'onglet Temps propose un export CSV du détail des saisies, et l'onglet FDR un export CSV des initiatives de l'année affichée (pour un tableur). Le rapport hebdomadaire, les flash reports projet (onglet Tâches) et les séances de COPIL proposent en plus un **export PowerPoint** — voir la section Sécurité pour le détail de ce qui a été vérifié avant de l'activer.
...
### Export PowerPoint (rapport hebdomadaire, flash reports projet, COPIL)
...
- **Aucun objet OLE, aucune image, aucun lien hypertexte externe** : le code de génération (`src/lib/pptxCommon.ts`, `weeklyReportPptx.ts`, `flashReportPptx.ts`, `copilPptx.ts`) n'utilise que du texte et des tableaux mis en forme — vérifiable directement dans ces fichiers.
